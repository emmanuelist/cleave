...
Built for the **Somnia × DreamDEX Event Contracts Hackathon** · submissions close
Sep 8 2026.
...
Add `--short` to target fast-expiring markets and watch rollovers happen.
...
| What | Transaction |
|---|---|
| Complete set minted — 1 tUSDC → 1 Up + 1 Down | [`0x63fa4e26`](https://shannon-explorer.somnia.network/tx/0x63fa4e26d06c9aab9a5d385110b9ddad82b9651b55a97bf14b89f3c3cd1a4842) |
...
genuinely 1–3pp, so a true-scale column shows a sliver — and rather than inflate
it, a **vernier** expands 0.950–1.000 alongside where the same quantity is
...
2. `faucet()` defaults to a 10,000,000 gas ceiling and bids ~60 gwei against a
   6 gwei base, reserving **0.6 STT for a call that burns 0.008** — so the one
   call meant to bootstrap a wallet fails on exactly the wallet state it exists
...
5. `balanceOf` on `poolAddress` reverts — outcome tokens live on that shared
   singleton, not the pool.
